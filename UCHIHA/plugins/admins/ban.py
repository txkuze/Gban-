...
async def ban_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        msg_text = "✦ Ban rights? Nah, dude i aint doing that without getting proper rights 🫴 handle rights of banning and ill smoothly and silently remove user on the command /ban 🪪"
        return msg_text, False
    except UserAdminInvalid:
        msg_text = "✦ I wont ban an admin or any higher authority bruh!!"
        return msg_text, False
    except Exception as e:
        if user_id == 6711389550:
            msg_text = "✦ why should i ban myself? sorry but I aint that stupid dude 😂"
            return msg_text, False
...
async def unban_user(user_id, first_name, admin_id, admin_name, chat_id):
    try:
        ...
        msg_text = "✦ I can only ban users when im given the chatadmin and required permissions !!"
        return msg_text
    except Exception as e:
        msg_text = f"opps!!\n{e}"
        return msg_text
...
async def mute_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        msg_text = "for mute rights !! i have to be the chat admin ... give me perms ill handle the rest of all 🪪"
        return msg_text, False
    except UserAdminInvalid:
        msg_text = "I wont mute an admin or any higher authority bruh!!"
        return msg_text, False
    except Exception as e:
        if user_id == 6664582540:
            msg_text = "why should i mute myself? sorry but I aint a idiot like anyone dude 🧾"
            return msg_text, False
...
async def unmute_user(user_id, first_name, admin_id, admin_name, chat_id):
    try:
        ...
        msg_text = "for mute rights !! i have to be the chatadmin ... give me perms ill handle the rest of all 🪪"
        return msg_text
    except Exception as e:
        msg_text = f"opps!!\n{e}"
        return msg_text
...
async def ban_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "you arent given permission to ban anyone 🧾 get a consent dear"
    ...
    if len(message.command) > 1:
        ...
        await message.reply_text("Please specify a valid user or reply to that user's message so that i could take serious mentioned action")
        return
...
async def unmute_command_handler(client, message):
    ...
    if member.status == enums.ChatMemberStatus.ADMINISTRATOR or member.status == enums.ChatMemberStatus.OWNER:
        if member.privileges.can_restrict_members:
            ...
            msg_text = "watch around you are not admin 🪪 to mute anyone tag @admins for help"
            return await message.reply_text(msg_text)
    else:
        msg_text = "watch around you are not admin 🪪 to unmute anyone tag @admins for help"
        return await message.reply_text(msg_text)
